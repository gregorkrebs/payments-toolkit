  ...
  function init() {
    ...
    searchInput.addEventListener('input', async (e) => {
      ...
      try {
        ...
        if (results.length === 0) {
          searchResults.innerHTML = '<div style="padding: 0.75rem; color: var(--text-muted); font-size: 0.9em;">Keine Ergebnisse gefunden.</div>';
        } else {
          searchResults.innerHTML = results.map(b => `
            <div class="bank-search-item" style="padding: 0.5rem 0.75rem; border-bottom: 1px solid var(--border); cursor: pointer;" 
                 data-blz="${esc(b.blz)}" data-bic="${esc(b.bic||'')}" data-name="${esc(b.name)}" data-plz="${esc(b.plz||'')}" data-city="${esc(b.city||'')}">
              <strong style="display:block; font-size: 0.95em; line-height: 1.2;">${esc(b.name)}</strong>
              <div style="font-size: 0.85em; color: var(--text-muted); margin-top: 0.2rem; display: flex; gap: 1rem;">
                <span>BLZ: ${esc(b.blz)}</span>
                <span>BIC: ${b.bic ? esc(b.bic) : '-'}</span>
                <span>Ort: ${b.plz ? esc(b.plz) + ' ' : ''}${b.city ? esc(b.city) : '-'}</span>
              </div>
            </div>
          `).join('');
          ...
          searchResults.querySelectorAll('.bank-search-item').forEach(item => {
            ...
            item.addEventListener('click', () => {
              ...
              resultEl.innerHTML = statusBox(true, 'Bank gefunden', `
                <table class="data-table" style="margin-top: 0.5rem;">
                  <tbody>
                    <tr><td style="width:140px;color:var(--text-muted)">Bank</td><td><strong>${esc(d.name)}</strong></td></tr>
                    <tr><td style="color:var(--text-muted)">BLZ</td><td>${esc(d.blz)}</td></tr>
                    <tr><td style="color:var(--text-muted)">BIC</td><td>${d.bic ? esc(d.bic) : '-'}</td></tr>
                    <tr><td style="color:var(--text-muted)">Ort</td><td>${d.plz ? esc(d.plz) + ' ' : ''}${d.city ? esc(d.city) : '-'}</td></tr>
                  </tbody>
                </table>
              `);
              searchResults.style.display = 'none';
