  ...
  const modules = [
    {
      ...
      desc: 'PAIN-XML (001/002/008) und DTAZV-Dateien auf Gueltigkeit pruefen. Fehlerfelder werden exakt benannt.',
    ...
    },
    {
      ...
      desc: 'Auftragsart + Regelwerk auswaehlen, Formular ausfuellen, XML-Datei generieren und direkt validieren.',
    ...
    },
    {
      page: 'tools',
      title: 'IBAN-Tools',
      desc: 'IBAN pruefen, berechnen (Land+BBAN), DE-Komfortmodus (BLZ+Konto), IBAN zerlegen, Batch-Pruefung.',
    ...
    },
    {
      page: 'samples',
      title: 'Beispieldaten',
      desc: 'STA (MT940) oder C53 (CAMT.053 XML) Beispieldateien mit 1-250 zufaelligen Umsaetzen aus 250 Mustertransaktionen generieren.',
  ...
  function render() {
    ...
    grid.innerHTML = modules.map(m =>
      `<div class="card">
        ...
        <h3>${esc(m.title)}</h3>
        <p>${esc(m.desc)}</p>
        <a href="#${m.page}" class="card-link">Oeffnen</a>
      </div>`
    ).join('');
  }
